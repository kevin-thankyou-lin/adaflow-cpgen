...
class IKWrapper:
    def __init__(self):
        self.model = mujoco.MjModel.from_xml_path("adaflow/real_world/assets/franka_emika_panda/scene.xml")
    ...
    @staticmethod
    def parse_rotation(rotation_input: Union[np.ndarray, List[float]]) -> Tuple[Rotation, np.ndarray]:
        """
        Converts the input rotation to a SciPy Rotation instance and its 3x3 matrix.
        Accepts:
          - 3x3 matrix,
          - 4-element quaternion,
          - 3-element axis-angle vector (angle in radians encoded in the norm).
        """
        rot = np.array(rotation_input)
        if rot.shape == (3, 3):
            r = Rotation.from_matrix(rot)
            return r, r.as_matrix()
        elif rot.shape == (4,):  # note: scipy assumes quats are xyzw, while mujoco asssumes wxyz
            ...
            raise ValueError("Invalid rotation input. Must be a 3x3 matrix, 4-element quaternion, or 3-element axis-angle vector.")

    def ik_trajectory_to_target_pose(self, target_pos: np.ndarray, target_rotation: Union[np.ndarray, List[float]],
        # f: Literal["axis-angle", "quat_wxyz"],
        start_joint_positions: List[float], num_points: int = 100,
        verbose: bool = False
    ):
        aa = time.time()
        ...
        # Get current and target rotations as SciPy Rotation objects.
        current_rot = Rotation.from_matrix(current_mat)
        target_rot, target_mat = self.parse_rotation(target_rotation)
        ...
        # Create a Slerp object for rotation interpolation.
        key_times = np.array([0, 1])
        key_rots = Rotation.from_quat(np.array([current_rot.as_quat(), target_rot.as_quat()]))
        slerp = Slerp(key_times, key_rots)


        # Precompute interpolation factors.
        times_interp = np.linspace(1/num_points, 1, num_points)
        interp_rots = slerp(times_interp)

        cc = time.time()
        # print(f"step ik_trajectory_to_target_pose 1 {bb-aa}")
        # print(f"step ik_trajectory_to_target_pose 2 {cc-bb}")
        for i, t in enumerate(times_interp):
            dd = time.time()
            pos = current_pos + (target_pos - current_pos) * t
            interp_mat = interp_rots[i].as_matrix()
            predicted_joints = self.inverse_kinematics(self.model, self.data, interp_mat, pos, start_joint_positions)
            predicted_joints_seq.append(predicted_joints)
            ee = time.time()
            # print(f"ik time: {ee - dd}")
    ...
    def inverse_kinematics(self, model, data, target_mat, target_pos, reset_joint_positions, max_steps: int = 25, err_thresh: float = 1e-5):
    ...
    def interpolate_dense_traj(self, joint_seq, minimal_displacement=0.005):
        ...
        for i in range(len(joint_seq) - 1):
            ...
            if max_joint_displacement < minimal_displacement:
                ...
                # new_gripper_actions += np.repeat(gripper_actions[i], num_points)
                repeat_info.append(num_points + 1)
        new_joint_seq.append(joint_seq[-1])
        repeat_info.append(1)
        print("increased joint sequence from {} to {}".format(len(joint_seq), len(new_joint_seq)))
        return new_joint_seq, repeat_info
