...
class TrainDiffusionUnetImageWorkspace(BaseWorkspace):
    ...
    def run(self):
        ...
        # if cfg.normalize_pixel:
        #     self.model.update_transform(dataset.per_pixel_mean, dataset.per_pixel_std)
        #     if cfg.training.use_ema:
        #         self.ema_model.update_transform(dataset.per_pixel_mean, dataset.per_pixel_std)
        ...
        # newly added for documenting wandb run urls of source datasets #
        with h5py.File(cfg.task.dataset['dataset_path'], "r") as src_file:
            wandb_run_urls = src_file.attrs.get("wandb_run_url", [])
            if isinstance(wandb_run_urls, str):
                wandb_run_urls = [wandb_run_urls]
            if len(wandb_run_urls) > 0:
                for i, url in enumerate(wandb_run_urls):
                    wandb_run.config.update({f"wandb_run_url_{i}": url})
